...
移植自 jev-chat/jev-chat-jarvis 的 tools/jev/questions.py（MIT 协议，见根目录 NOTICE）。
题目的 instructions 和 criteria 用英文写，聊天内容保留中文原文，这是 Laya 和 Jev 共同的用法。

和原版的差别只有一处：原来 3 道 noul（是非）题改成了两个选项的 choice 题。
原因是 Laya 的 issue #156：noul 的选项标签在模型内部被写死为 false / true，
模型对这两个词本身有偏见，会不看内容地永远答否。所以这里的选项名刻意避开
true / false / yes / no，用 literal / subtext 这类有含义的词。
"""

BACKGROUND_NOTE = " Facts given in background are provided context, not off-topic."

# 三道原本是是非题的题目：(代表「是」的选项名, 代表「否」的选项名)
BOOL_LABELS = {
    "literal_question": ("literal", "subtext"),
    "should_reply_now": ("substance_now", "hold"),
    "tension_resolved": ("resolved", "unresolved"),
}
...
JUDGE_QUESTIONS = {
    ...
    "should_reply_now": {
        ...
        "instructions": (
            ...
            "Choose hold if the thing they want you to recite or prove is not present in this snippet "
            "(you would be guessing). 'Then say it' / 'you better' while you are stalling means hold. "
            "Choose hold if they already accepted and closed the topic. "
            "Choose substance_now only if the needed fact, plan, or named fault is already in this snippet."
        ),
        "criteria": {
            "substance_now": (
    ...
    "tension_resolved": {
        ...
        "instructions": (
            ...
            "Choose resolved only if there was never tension, or the other person has clearly accepted, "
            "cooled down, joked again, or said it is fine. "
            "A sarcastic 'you better', an unanswered test, leftover blame, or an open ultimatum means unresolved."
        ),
        "criteria": {
            "resolved": (
...
# 每道题末尾都加一句，告诉模型 background 字段是给定的背景，别当成跑题内容扣分。
for _q in JUDGE_QUESTIONS.values():
    _q["instructions"] = _q["instructions"] + BACKGROUND_NOTE

_SIDE_ALIASES = {
    "me": "me", "我": "me", "self": "me",
    "other": "other", "her": "other", "him": "other", "them": "other", "对方": "other",
}


def normalize_messages(messages: list) -> list:
    """把各种写法统一成 [{"from": "me"|"other", "text": str}]，只保留最近 10 条。"""
    cleaned = []
    for item in messages:
        ...
        side = _SIDE_ALIASES.get(str(who).strip().lower(), _SIDE_ALIASES.get(str(who).strip()))
        if side is None:
            raise ValueError(f"消息的 from 只能是 me 或 other，收到 {who!r}")
        text = str(text).strip()
        if text:
            cleaned.append({"from": side, "text": text})
    return cleaned[-10:]


def build_state(messages: list, relationship: str, background: str = "") -> dict:
    """组装发给 Laya 的 state。relationship 是一句话，例如「对方是我的伴侣」。"""
    ...
    state = {
        "chat": {
            ...
            "messages": cleaned,
            "latest_from": cleaned[-1]["from"],
        }
    }
    if background and background.strip():
        state["background"] = background.strip()
    return state


RANK_KEYS = ("reply_a", "reply_b", "reply_c", "reply_d", "reply_e")


def build_rank_question(candidates: list) -> dict:
    """排序题：候选回复原文当选项。2 到 5 条。"""
    if not 2 <= len(candidates) <= len(RANK_KEYS):
        raise ValueError(f"候选回复要 2 到 {len(RANK_KEYS)} 条，收到 {len(candidates)} 条")
    return {
        "best_reply": {
            "type": "choice",
            "instructions": (
            ...
            "criteria": {key: str(text) for key, text in zip(RANK_KEYS, candidates)},
        }
    }
