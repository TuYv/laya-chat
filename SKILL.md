---
name: laya-chat
description: 安装向导。用户说「安装 laya-chat」「装一下微信回复副驾」「laya chat 怎么装」时使用。带用户在 macOS 上装好 Laya Chat 悬浮窗程序：建环境、开权限、选求助工具、下权重、自检、设开机启动。装好之后程序独立运行，不再需要 AI 助手。
---
...
这个 skill 的根目录记为 `$R`（SKILL.md 所在目录）。
